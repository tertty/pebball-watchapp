#include <pebble.h>
#include "session_id_select_window.h"
...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      text_layer_set_text(pin_window->main_text, "Input Session ID:");
      ...
      text_layer_set_text(pin_window->sub_text, "Visit a Pebball! website to continue");
...
static void pin_complete_callback(PIN pin, void *context) {
    ...
    static char combined_pin_num_buffer[5];

    snprintf(combined_pin_num_buffer, sizeof(combined_pin_num_buffer), "%d%d%d", pin.digits[0], pin.digits[1], pin.digits[2]);

    construct_outgoing_app_message(MESSAGE_KEY_WEBSOCKET_CONNECT_EVENT, ((pin.digits[0] * 100) + (pin.digits[1] * 10) + (pin.digits[2])));
}

void session_id_select_window_push() {
